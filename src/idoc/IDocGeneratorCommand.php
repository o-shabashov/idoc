...
class IDocGeneratorCommand extends Command
{
     ...
     * @return void
     */
    public function handle(IDocGenerator $generator)
    {
    ...
    {
        $result = $routes->map(function ($routeGroup, $groupName) {

            return collect($routeGroup)->map(function ($route) use ($groupName, $routeGroup) {

                $methodGroup = $routeGroup->where('uri', $route['uri'])->mapWithKeys(function ($route) use ($groupName) {
                    ...
                    $pathParameters = collect($route['pathParameters'] ?? [])->map(function ($schema, $name) {
                        return [
                            'in'          => 'path',
                            'name'        => $name,
                            'description' => $schema['description'],
                    ...
                    $headerParameters = collect($route['headers'])->map(function ($value, $header) {
        ...
        $collection = [
            ...
            'components' => [
                ...
                'schemas' => $routes->mapWithKeys(function ($routeGroup, $groupName) {
                    ...
                    return collect($routeGroup)->mapWithKeys(function ($route) {
